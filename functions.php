...
function mytesttheme_setup()
{
	/*
		...
		* If you're building a theme based on mytesttheme, use a find and replace
		* to change 'mytesttheme' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__('Primary', 'mytesttheme'),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'mytesttheme_custom_background_args',
			array(
...
function mytesttheme_scripts()
{
	...
	wp_enqueue_style('mytesttheme-style', get_stylesheet_uri(), array(), _S_VERSION);
	wp_style_add_data('mytesttheme-style', 'rtl', 'replace');

	wp_enqueue_script('mytesttheme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true);
